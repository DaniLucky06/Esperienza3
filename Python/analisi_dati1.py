...
# Estrazione variabili per comodità
x = df['massa']
y = df['periodo']
...
# Test del Chi-Quadrato per la media pesata
...
# Verifica compatibilità pendenza con zero
t_student = abs(B) / sigma_B
print(f"Compatibilità della pendenza con 0: {t_student:.2f} deviazioni standard (sigma)")

if t_student < 3:
    print("Esito: La pendenza è compatibile con zero (T indipendente dalla massa).")
    # Limite superiore (valore assoluto di B + 3 sigma)
    limite_sup = abs(B) + 3 * sigma_B
    print(f"Limite superiore per la dipendenza: {limite_sup:.2e} s/g\n")
else:
...
# Dati sperimentali con barre di errore
plt.errorbar(x, y, yerr=sig_y, fmt='o', capsize=5, color='black', label='Dati sperimentali')

# Retta del modello a periodo costante (media pesata)
plt.axhline(y=media_pesata, color='green', linestyle='--', alpha=0.7, 
            label=f'Media pesata ({media_pesata:.3f} s)')

# Retta della regressione lineare
x_range = np.linspace(x.min(), x.max(), 100)
y_fit = A + B * x_range
plt.plot(x_range, y_fit, color='red', alpha=0.7, 
         label=f'Fit lineare (pendenza = {B:.1e})')
